...
const productos = [
    ...
    // COMBOS
      { id: 3, cat: "Combos", nombre: "Barquito Premium 40", precio: 55000, ingredientes: "Surtido de Rolls fríos. Contiene 8 Red Pasion, 8 Ebi Furai, 8 Karaage Roll, 8 Tuna Deluxe y 8 Sweet Kani. Incluye 4 juegos de palillos y 4 salsas de soja y 2 de maracuyá.", imagen: "imagenes/Combo1.jpg", piezas: "40 Piezas", popular: true },
...
function filtrar(cat) {
    ...
    document.querySelectorAll('.category-btn').forEach(btn => {
        ...
        if (btnText === cat.toLowerCase() || (cat === 'Hot' && btnText === 'rolls calientes') || (cat === 'Frios' && btnText === 'rolls fríos')) {
...
function render() {
    ...
    grid.innerHTML = filtrados.map(item => `
        <div class="sushi-card glass rounded-3xl overflow-hidden flex flex-col h-full animate-fade">
            ...
            <div class="p-6 flex flex-col flex-grow text-center md:text-left">
                ...
                <button onclick="agregarAlCarrito(${item.id})" 
                        class="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-2xl text-[11px] font-bold uppercase tracking-wider transition-all shadow-lg active:scale-95 shadow-orange-900/10">
                    Añadir al Carrito
                </button>
            </div>
        </div>
